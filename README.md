...
* Once you have your `.npmrc` and before you run `npm install` command, replace all occurrences of `@josephboadi/joy-jobber-shared` with your own shared library.
* After replacing all occurrences of `@josephboadi/joy-jobber-shared`, you can then run `npm install` command.
* Copy contents of `.env.dev` to `.env` file
  * In the `DATABASE_HOST` env variable, use your own ip as its value.
  * You can generate a new `GATEWAY_JWT_TOKEN` and `JWT_TOKEN` from date | md5
...
* Steps to build and push your image to docker hub
  * `docker build -t jboadi/spepas-gateway .`
  * `docker tag jboadi/spepas-gateway jboadi/spepas-gateway:stable`
  * `docker push jboadi/spepas-gateway:stable`
